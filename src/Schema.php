...
use AndrewSouthwell\Migrate\Model;
 ...
 */
    public function build($data) {
        ...
        if (file_exists($this->path .'/'. $file) && is_file($this->path .'/'. $file)) {
            $this->file = $file;
        } elseif (!empty($this->plugin)) {
            $this->path = CakePlugin::path($this->plugin) . 'Config' .'/'. 'Schema';
        }
    }
 ...
 */
    public function load($options = array()) {
        ...
        include('../storage/app/schema/database.php');
        ...
        $this->build($options);
        $class = 'DatabaseSchema'; //$this->name . 'Schema';
 ...
 */
    public function write($object, $options = array()) {
        ...
        // $file = new File('../storage/schema/' . $options['file'], true);
        $content = "<?php \n{$out}";
        // if ($file->write($content)) {
        //     return $content;
        // }

        file_put_contents('../storage/app/schema/database.php', $content);
        return false;
    }
 ...
 */
    protected function _requireFile($path, $file) {
        if (file_exists($path .'/'. $file) && is_file($path .'/'. $file)) {
            ...
            return true;
        } elseif (file_exists($path .'/'. 'schema.php') && is_file($path .'/'. 'schema.php')) {
